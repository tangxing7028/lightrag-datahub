...
- **MinerU**：`MINERU_API_MODE`（`local` / `official`）、`MINERU_API_TOKEN`、`MINERU_LOCAL_ENDPOINT` 或 `MINERU_OFFICIAL_ENDPOINT`，可选 `MINERU_ENGINE_VERSION` / `MINERU_MODEL_VERSION` / `MINERU_POLL_INTERVAL_SECONDS` / `MINERU_MAX_POLLS` / `MINERU_HTTP_TIMEOUT_SECONDS` / `MINERU_CONNECT_TIMEOUT_SECONDS`。`REMOTE_PARSER_TIMEOUT` 可作为 HTTP 超时回退配置。
